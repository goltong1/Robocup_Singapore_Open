...
void Game0()
{
    ...
    else if(Time>=181 && Time<=1000&&(LoadedObjects<2&&(Time>tDeposit+2)))
    {
        Duration = 0;
        CurAction =2;
    }
...
if(Duration<2&&abs(TargetTheta-Compass)<5)//until ?? Degrees
{WheelLeft=3;
                    WheelRight=3;
                    //Duration=1;
...
(CSRight_R>175&&CSRight_G<50&&CSRight_B<50)))
    ...
    else if((US_Left<10||US_Right<10||US_Front<20))
    {
        ...
        else if(US_Front<14||US_Left<10||US_Right<10)
        {
            Duration = 0;
            CurAction =19;
        }
...
if(Duration==47){printf("D-Green:%d-%d\n",CSLeft_G,CSRight_G);
                    }
if(Duration==46){printf("D-Blue:%d-%d\n",CSLeft_B,CSRight_B);
                    }

if(Duration<47){WheelLeft=0;
                    WheelRight=0;
                    }
