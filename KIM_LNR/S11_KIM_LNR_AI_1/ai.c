...
{
    char info[1024];
    sprintf(info, "Duration=%d;SuperDuration=%d;bGameEnd=%d;CurAction=%d;CurGame=%d;SuperObj_Num=%d;SuperObj_X=%d;SuperObj_Y=%d;Teleport=%d;LoadedObjects=%d;wHead=%d;tForward=%d;CornerStress=%d;TargetTheta=%d;ModeEscape=%d;Steering=%d;bInitial=%d;WTC=%d;tDeposit=%d;tLoad=%d;NowDirection=%d;wDirection=%d;bSwamp=%d;dCollectionBox=%d;NeedY=%d;NeedX=%d;DX=%d;DY=%d;GuessX=%d;GuessY=%d;CountPush=%d;TX=%d;TY=%d;Mark=%d;US_Front=%d;US_Left=%d;US_Right=%d;CSLeft_R=%d;CSLeft_G=%d;CSLeft_B=%d;CSRight_R=%d;CSRight_G=%d;CSRight_B=%d;PositionX=%d;PositionY=%d;TM_State=%d;Compass=%d;Time=%d;WheelLeft=%d;WheelRight=%d;LED_1=%d;MyState=%d;",Duration,SuperDuration,bGameEnd,CurAction,CurGame,SuperObj_Num,SuperObj_X,SuperObj_Y,Teleport,LoadedObjects,wHead,tForward,CornerStress,TargetTheta,ModeEscape,Steering,bInitial,WTC,tDeposit,tLoad,NowDirection,wDirection,bSwamp,dCollectionBox,NeedY,NeedX,DX,DY,GuessX,GuessY,CountPush,TX,TY,Mark,US_Front,US_Left,US_Right,CSLeft_R,CSLeft_G,CSLeft_B,CSRight_R,CSRight_G,CSRight_B,PositionX,PositionY,TM_State,Compass,Time,WheelLeft,WheelRight,LED_1,MyState);
    return info;
}
...
(CSRight_R>50&&CSRight_R<70&&CSRight_G>86&&CSRight_G<119&&CSRight_B>210)))
    ...
    else if((LoadedObjects>0)&&((CSLeft_R>190&&CSLeft_G>200&&CSLeft_B<40)||
(CSRight_R>190&&CSRight_G>200&&CSRight_B<40))
    ...
    else if(LoadedObjects<6&&((CSLeft_R<60&&CSLeft_G<60&&CSLeft_B<60)
||(CSRight_R<60&&CSRight_G<60&CSRight_B<60)))
    {
        Duration = 44;
        CurAction =6;
    }
...
(CSRight_R>200&&CSRight_G>200&&(CSRight_B<80&&CSRight_B>41))
    ...
    else if(CSLeft_R>=190 && CSLeft_R<=255 && CSLeft_G>=155 && CSLeft_G<=195 && CSLeft_B>=0 && CSLeft_B<=20 && CSRight_R>=190 && CSRight_R<=255 && CSRight_G>=155 && CSRight_G<=195 && CSRight_B>=0 && CSRight_B<=20&&(LoadedObjects>0))
    {
        Duration = 49;
        CurAction =10;
    }
    else if(CSLeft_R>=190 && CSLeft_R<=255 && CSLeft_G>=155 && CSLeft_G<=195 && CSLeft_B>=0 && CSLeft_B<=30&&(LoadedObjects>0))
    {
        Duration = 0;
        CurAction =11;
    }
    else if(CSRight_R>=190 && CSRight_R<=255 && CSRight_G>=155 && CSRight_G<=195 && CSRight_B>=0 && CSRight_B<=30&&(LoadedObjects>0))
    {
        Duration = 0;
        CurAction =12;
    }
    else if(LoadedObjects>=3&&(((CSLeft_R<200&&CSLeft_R>40)&&(CSLeft_G>50&&CSLeft_G<100)&&CSLeft_B>230)||((CSRight_R<200&&CSRight_R>40)&&(CSRight_G>50&&CSRight_G<100)&&CSRight_B>230))&&Mark==0)
    {
        Duration = 0;
        CurAction =13;
    }
    else if(LoadedObjects>=3&&((CSLeft_R<35&&(CSLeft_G>200)&&CSLeft_B>230)||(CSRight_R<35&&(CSRight_G>200)&&CSRight_B>230))&&Mark==0)
    {
        Duration = 0;
        CurAction =14;
    }
...
if(Duration<3){WheelLeft=-1;
                    WheelRight=-4;
                    Duration=1;
                    }

if((CSLeft_G>210||CSLeft_B>100)&&(CSRight_G>210||CSRight_B>100))
{Duration=0;
                    tDeposit=Time;
                    printf("yeh Deposit!!?\n");
                    }
...
||(CSLeft_R>180&&CSRight_G<15&&CSRight_B>230))
    ...
    else if((PositionX!=0&&PositionY!=0)&&(abs(PositionX-SuperObj_X)<5&&abs(PositionY-SuperObj_Y)<5))
    {
        Duration = 44;
        CurAction =3;
    }
    else if(CSLeft_R>=120 && CSLeft_R<=165 && CSLeft_G>=130 && CSLeft_G<=175 && CSLeft_B>=170 && CSLeft_B<=220)
    {
        Duration = 0;
        CurAction =4;
    }
    else if(CSRight_R>=120 && CSRight_R<=165 && CSRight_G>=130 && CSRight_G<=175 && CSRight_B>=170 && CSRight_B<=220)
    {
        Duration = 0;
        CurAction =5;
    }
...
((CSLeft_R>190&&CSLeft_G>200&&CSLeft_B<20)||
(CSRight_R>190&&CSRight_G>200&&CSRight_B<20))
...
(CSRight_R>175&&CSRight_G<50&&CSRight_B<50)))
    ...
    else if(CSLeft_R>=190 && CSLeft_R<=255 && CSLeft_G>=150 && CSLeft_G<=195 && CSLeft_B>=0 && CSLeft_B<=20 && CSRight_R>=190 && CSRight_R<=255 && CSRight_G>=150 && CSRight_G<=195 && CSRight_B>=0 && CSRight_B<=20&&(LoadedObjects>0))
    {
        Duration = 49;
        CurAction =12;
    }
    else if(CSLeft_R>=190 && CSLeft_R<=255 && CSLeft_G>=150 && CSLeft_G<=195 && CSLeft_B>=0 && CSLeft_B<=30&&(LoadedObjects>0))
    {
        Duration = 0;
        CurAction =13;
    }
    else if(CSRight_R>=190 && CSRight_R<=255 && CSRight_G>=150 && CSRight_G<=195 && CSRight_B>=0 && CSRight_B<=30&&(LoadedObjects>0))
    {
        Duration = 0;
        CurAction =14;
    }
    else if(PositionX>=140 && PositionX<=211 && PositionY>=230 && PositionY<=270)
    {
        Duration = 0;
        CurAction =15;
    }
    else if(PositionX>=1 && PositionX<=1000 && PositionY>=215 && PositionY<=270&&(WTC==0&&(PositionX<18||PositionX>347||PositionY>252||PositionY<50)
))
    {
        Duration = 0;
        CurAction =16;
    }
    else if(PositionX>=1 && PositionX<=1000 && PositionY>=1 && PositionY<=204&&(WTC==0&&(PositionX<16||PositionX>344||PositionY<16||PositionY>190)
))
    {
        Duration = 0;
        CurAction =17;
    ...
    else if(true)
    {
        Duration = 0;
        CurAction =21;
        if(PositionX>=1 && PositionX<=153 && PositionY>=1 && PositionY<=27&&((LoadedObjects>3&&WTC==0)||(Time>460&&WTC==0&&LoadedObjects>0)))
        {
            Duration = 0;
            CurAction =22;
        }
        else if(PositionX>=181 && PositionX<=231 && PositionY>=78 && PositionY<=173&&((LoadedObjects>3&&WTC==0)||(Time>460&&WTC==0&&LoadedObjects>0)))
        {
            Duration = 0;
            CurAction =23;
        }
...
{WheelLeft=-2;
 ...
 //Set Avoid Degrees
 if(PositionX<192)
 {if(PositionY<60){TargetTheta=270;
                    }else{TargetTheta=330;
                    }}
 else
 {if(PositionY>82){TargetTheta=0;
                    }else{TargetTheta=160;
                    }}
 ...
 if(TargetTheta==330){if(Compass>180){wHead=-1;
                    }}
 if(TargetTheta==180){if(Compass<180){wHead=-1;
                    }}
 if(TargetTheta==20){if(Compass>200||Compass<20){wHead=-1;
                    }}
 if(TargetTheta==270){if(Compass>90&&Compass<270){wHead=-1;
                    }}
 if(TargetTheta==160){if(Compass>330||Compass<160){wHead=-1;
                    }}
