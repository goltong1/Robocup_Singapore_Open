...
{
    char info[1024];
    sprintf(info, "Duration=%d;SuperDuration=%d;bGameEnd=%d;CurAction=%d;CurGame=%d;SuperObj_Num=%d;SuperObj_X=%d;SuperObj_Y=%d;Teleport=%d;LoadedObjects=%d;wHead=%d;tForward=%d;CornerStress=%d;TargetTheta=%d;ModeEscape=%d;Steering=%d;bInitial=%d;WTC=%d;tDeposit=%d;tLoad=%d;Tx=%d;Ty=%d;NowDirection=%d;wDirection=%d;bSwamp=%d;dCollectionBox=%d;NeedY=%d;NeedX=%d;DX=%d;DY=%d;US_Front=%d;US_Left=%d;US_Right=%d;CSLeft_R=%d;CSLeft_G=%d;CSLeft_B=%d;CSRight_R=%d;CSRight_G=%d;CSRight_B=%d;PositionX=%d;PositionY=%d;TM_State=%d;Compass=%d;Time=%d;WheelLeft=%d;WheelRight=%d;LED_1=%d;MyState=%d;",Duration,SuperDuration,bGameEnd,CurAction,CurGame,SuperObj_Num,SuperObj_X,SuperObj_Y,Teleport,LoadedObjects,wHead,tForward,CornerStress,TargetTheta,ModeEscape,Steering,bInitial,WTC,tDeposit,tLoad,Tx,Ty,NowDirection,wDirection,bSwamp,dCollectionBox,NeedY,NeedX,DX,DY,US_Front,US_Left,US_Right,CSLeft_R,CSLeft_G,CSLeft_B,CSRight_R,CSRight_G,CSRight_B,PositionX,PositionY,TM_State,Compass,Time,WheelLeft,WheelRight,LED_1,MyState);
    return info;
}
...
((CSLeft_R>194&&CSLeft_G<20&&CSLeft_B<20)||
(CSRight_R>194&&CSRight_G<20&&CSRight_B<20)))
    {
        Duration = 46;
        CurAction =7;
    }
    else if(CSLeft_R>=190 && CSLeft_R<=255 && CSLeft_G>=70 && CSLeft_G<=110 && CSLeft_B>=0 && CSLeft_B<=10 && CSRight_R>=190 && CSRight_R<=255 && CSRight_G>=70 && CSRight_G<=110 && CSRight_B>=0 && CSRight_B<=10&&(LoadedObjects>0))
    {
        Duration = 45;
        CurAction =8;
    }
    else if(CSLeft_R>=190 && CSLeft_R<=255 && CSLeft_G>=70 && CSLeft_G<=120 && CSLeft_B>=0 && CSLeft_B<=30&&(LoadedObjects>0))
    {
        Duration = 0;
        CurAction =9;
    }
    else if(CSRight_R>=190 && CSRight_R<=255 && CSRight_G>=70 && CSRight_G<=120 && CSRight_B>=0 && CSRight_B<=30&&(LoadedObjects>0))
    {
        Duration = 0;
        CurAction =10;
    }
...
(CSRight_R>194&&CSRight_G<20&&CSRight_B<20)))
    ...
    else if(CSLeft_R>=180 && CSLeft_R<=255 && CSLeft_G>=60 && CSLeft_G<=120 && CSLeft_B>=0 && CSLeft_B<=30&&(LoadedObjects>0))
    {
        Duration = 0;
        CurAction =12;
    }
    else if(CSRight_R>=180 && CSRight_R<=255 && CSRight_G>=60 && CSRight_G<=120 && CSRight_B>=0 && CSRight_B<=30&&(LoadedObjects>0))
    {
        Duration = 0;
        CurAction =13;
    }
    ...
    else if(true)
    {
        Duration = 0;
        CurAction =18;
        if(PositionX>=50 && PositionX<=169 && PositionY>=1 && PositionY<=63&&((LoadedObjects>3&&WTC==0)||(Time>460&&WTC==0&&LoadedObjects>0)))
        {
            Duration = 0;
            CurAction =19;
        }
        else if(PositionX>=232 && PositionX<=320 && PositionY>=195 && PositionY<=1000&&((LoadedObjects>3&&WTC==0)||(Time>460&&WTC==0&&LoadedObjects>0)))
        {
            Duration = 0;
            CurAction =20;
        }
...
if(PositionY<130){TargetTheta=0;
                    }
else{if(PositionX<78){TargetTheta=90;
                    }else{TargetTheta=270;
                    }}
