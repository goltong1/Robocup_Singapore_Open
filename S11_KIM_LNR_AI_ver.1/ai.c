...
(CSRight_R>175&&CSRight_G<50&&CSRight_B<50)))
    ...
    else if(true)
    {
        ...
        else if(PositionX>=1 && PositionX<=200 && PositionY>=1 && PositionY<=40&&((LoadedObjects>3&&WTC==0)||(Time>460&&WTC==0&&LoadedObjects>0)))
        {
            Duration = 0;
            CurAction =22;
        }
...
{WheelLeft=-2;
 ...
 //Set Avoid Degrees
 if(PositionX<200)
 {if(PositionY<42){TargetTheta=250;
                    }else{TargetTheta=0;
                    }}
 else
 {if(PositionY<60){TargetTheta=150;
                    }else{TargetTheta=20;
                    }}
 ...
 if(TargetTheta==250){if(Compass>70&&Compass<250){wHead=-1;
                    }}
